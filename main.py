...
class Settings(BaseSettings):
    ...
    GITHUB_API_BASE: str = Field("https://api.github.com", env="GITHUB_API_BASE")
    GITHUB_PAGES_BASE: Optional[str] = None
    ALLOW_DEV_NO_SECRET: bool = Field(False, env="ALLOW_DEV_NO_SECRET")  # dev helper, default false
...
# Allow CORS during development so browser-based clients can call the API
app.add_middleware(
    CORSMiddleware,
...
# ------------------------- Utility -------------------------
def verify_secret(secret_from_request: str) -> bool:
    # allow bypass if ALLOW_DEV_NO_SECRET is true (local dev only)
    if settings.ALLOW_DEV_NO_SECRET:
        logger.info("[DEV] ALLOW_DEV_NO_SECRET is enabled — skipping secret verification")
        return True
    return secret_from_request == settings.STUDENT_SECRET
...
# Validation error handler (helps debugging 422 issues)
@app.exception_handler(RequestValidationError)
async def validation_exception_handler(request: Request, exc: RequestValidationError):
    # Log full validation info
    logger.warning(f"Validation error for {request.client.host if request.client else 'unknown'} {request.url}: {exc}")
    flush_logs()
    # Return a clear JSON body with details
    return JSONResponse(
...
# Lightweight debug endpoint to echo payloads (no secret check / no background work)
@app.post("/debug-ready")
async def debug_ready(payload: Any = Body(...)):
    """
    Simple echo endpoint for testing payloads in Thunder Client / Postman.
    Does NOT perform secret verification or background processing.
    Use this to verify the JSON structure before sending to /ready.
    """
    logger.info("[DEBUG-READY] Received debug payload")
    flush_logs()
